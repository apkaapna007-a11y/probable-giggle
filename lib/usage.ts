...
// Using 'any' to avoid generic incompatibilities across Supabase versions
 ...
 */
export async function checkUsage(supabase: SupabaseAny, userId: string) {
...
export async function checkProUsage(supabase: SupabaseAny, userId: string) {
  const { data: userData, error: userDataError } = await supabase
    .from("users")
